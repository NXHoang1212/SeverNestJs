...
export class ProductAdminController {
  ...
  async create(
    ...
    try {
      const response = await this.productAdminService.createWithImage(
        body,
        imageFile,
      );
      return res.status(HttpStatus.OK).json(response);
  ...
  async detail(
    ...
    try {
      ...
      return res
        ...
        .json({ data: response, categories: categories });
    } catch (error: any) {
      return res.status(HttpStatus.BAD_REQUEST).json(error);
    }
  }
  ...
  async update(
    ...
    try {
      const response = await this.productAdminService.updateWithImage(
        id,
        body,
        file,
      );
