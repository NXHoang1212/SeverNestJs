...
export class FavouritesService {
    ...
    async createFavourites(request: FavouritesRequest): Promise<FavouritesResponse> {
        try {
    ...
    async getFavourites(id: string): Promise<FavouritesResponse> {
        try {
            const result = await this.favouritesModel.find({ UserId: id })
                .populate('UserId', '_id name mboile')
                .populate('ProductId', '_id name price image description category size topping');
            const reponseFavourites: FavouritesResponse = {
    ...
    async deleteFavourites(id: string): Promise<FavouritesResponse> {
        try {
            const result = await this.favouritesModel.findOneAndDelete({ ProductId: id });
            const reponseFavourites: FavouritesResponse = {
