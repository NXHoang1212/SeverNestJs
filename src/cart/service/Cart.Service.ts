...
export class CartSerivce {
    ...
    async getCartId(id: string): Promise<GetCartResponse> {
        try {
            const result = await this.CartModel.findOne({ UserId: id })
                .populate('UserId', '_id name mobile').populate({ path: 'ProductId.ProductId', model: 'Product', populate: { path: 'category', select: '_id name' } });
    ...
    async updateStatusProductInCart(userId: string, request: any): Promise<GetCartResponse> {
        try {
            ...
            const updatedCart = await this.CartModel.findOneAndUpdate({ UserId: userId }, { ProductId: updatedProduct }, { new: true });
            const response: GetCartResponse = {
                status: true,
