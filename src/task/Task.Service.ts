...
export class TasksService {
    constructor(private readonly promotionadminService: PromotionAdminService, private schedulerRegistry: SchedulerRegistry) { }
    private readonly logger = new Logger(TasksService.name);
    @Cron(CronExpression.EVERY_DAY_AT_MIDNIGHT)
    async handleCron() {
        this.logger.debug('Đã chạy cron job vào lúc 00:00');
        const deleteExpired = await this.promotionadminService.deletePromotionExpired();
        if (deleteExpired.status === true) {
