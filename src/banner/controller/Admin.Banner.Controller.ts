import { Controller, Get, Post, Delete, Body, Param, Res, HttpStatus, Query, UseInterceptors, UploadedFile, Req, Render, Put } from "@nestjs/common";
...
export class BannerAdminController {
    ...
    async create(@Body() body: AddBannerRequest, @Res() res: Response, @UploadedFile() imageFile: Express.Multer.File) {
        try {
    ...
    @Put('UpdateBaner/:id')
    @UseInterceptors(FileInterceptor('image', MulterConfig))
    async update(@Param('id') id: string, @Body() body: AddBannerRequest, @UploadedFile() imageFile: Express.Multer.File, @Res() res: Response) {
        try {
            const response = await this.bannerAdminSerivce.update(id, body, imageFile);
            return res.status(HttpStatus.OK).json(response);
        } catch (error: any) {
            return res.status(HttpStatus.BAD_REQUEST).json(error);
        }
